...
describe('Schema Validator', () => {
  ...
  it('should validate using custom validation rules', () => {
    const userSchema = new Schema({
      username: {
        type: 'string',
        required: true,
    ...
    const result = userSchema.validate({ username: 'invalid username!' });

    expect(result.valid).toBe(false);
    expect(result.errors?.[0].message).toBe('Username must only contain alphanumeric characters and underscores.');
  });
});
